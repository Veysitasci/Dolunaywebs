...
async def primary_cb(client: WebshotBot, callback_query: CallbackQuery):
    await callback_query.answer("İsteğinizi işleme alma.\Processing your request.")
    message = await callback_query.message.edit("**İşleniyor...**\Processing...")
    ...
    await message.edit("**Lütfen bekleyin, sıra sizde...\Please wait you are in a queue...**")
    try:
        future, wait_event = client.new_request(printer, callback_query.message.chat.id)
        await wait_event.wait()
        await message.edit(
            "**Web sitesini oluşturma...\Rendering the website...**",
        ...
        printer.cleanup()
        return
    await message.edit("**Yükleniyor/Loading...**")
    ...
    await asyncio.gather(
        message.delete(),
        message.reply_text('__Çıktıda içerik yoksa lütfen "Scroll Site" ayarını değiştirin.\__Please toggle "Scroll Site" setting if the output has no content.__'),
    )
    printer.cleanup()
...
async def release_cb(client: WebshotBot, callback_query: CallbackQuery):
    ...
    if event is not None:
        event.set()
        await callback_query.answer("Şimdi oluşturuluyor...\Rendering now...")
        await callback_query.message.edit_reply_markup()
    else:
        await callback_query.answer("Lütfen bekleyin\Please wait")
...
async def resolution_cb(_, callback_query: CallbackQuery, render_type: RenderType):
    ...
    await message.edit(text="Tercih edilen ayarları seçin.\Choose the prefered settings.", reply_markup=message.reply_markup)  # type: ignore
...
async def format_cb(_, callback_query: CallbackQuery, render_type: RenderType):
    ...
    await message.edit(text="Tercih edilen ayarları seçin.\Choose the prefered settings.", reply_markup=reply_markup)
...
async def options_cb(_, callback_query: CallbackQuery, render_type: RenderType):
    ...
    await callback_query.answer(f"{'opening' if toggled else'closing'} ek ayarlar/additional settings")
    if toggled:
    ...
    await callback_query.message.edit(text="Tercih edilen ayarları seçin.\Choose the prefered settings.", reply_markup=reply_markup)
...
async def configurations_cb(_, callback_query: CallbackQuery):
    ...
    if cb_data == "splits":
        ...
        await callback_query.answer("Talebinizi iptal ettim..!\Canceled your request..!")
